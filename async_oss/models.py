...
import logging
import copy
from oss2.utils import http_to_unixtime, make_progress_adapter, b64encode_as_string, \
    b64decode_from_string
from oss2.exceptions import ClientError, InconsistentError
from oss2.compat import urlunquote, to_string, urlquote
    ...
    def __init__(self, resp, progress_callback=None, crc_enabled=False, crypto_provider=None, discard=0):
        ...
        if self.__crypto_provider:
            ...
            if OSS_CLIENT_SIDE_ENCRYPTION_KEY in resp.headers or DEPRECATED_CLIENT_SIDE_ENCRYPTION_KEY in resp.headers:
                logger.warning("Using Bucket to get an encrypted object will return raw data,"
...
class ReplicationRule(object):
    ...
    def __init__(self,
        ...
        if sse_kms_encrypted_objects_status is not None and sse_kms_encrypted_objects_status not in [self.ENABLED,
                                                                                                     self.DISABLED]:
